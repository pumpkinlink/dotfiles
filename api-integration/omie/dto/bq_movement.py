...
def get_root_category(
    ...
    def compare_root_category(cc: categorias.CategoriaCadastro, code: str):
        if "codigo" not in cc:
            return False
    ...
    try:
        ...
        result = next(
            filter(lambda cc: compare_root_category(cc=cc, code=mc), categories), {}
        )
...
def get_categories_array(
    ...
    def compare_array_category(
        cc: categorias.CategoriaCadastro, mc: movimentos_financeiros.Categoria
    ):
        try:
...
def get_departments_array(
    ...
    def compare_array_department(
        dd: departamentos.Departamento, md: movimentos_financeiros.Departamento
    ):
        try:
    ...
    def build_bq_dep(md: movimentos_financeiros.Departamento) -> BQDepartment:
        result = next(
            filter(lambda dd: compare_array_department(dd=dd, md=md), departments), {}
        )
        desc = result["descricao"] if "descricao" in result else None
...
def get_account_name(
    movement: movimentos_financeiros.Movimento, accounts: list[ContaCorrente]
) -> str | None:
    try:
...
def get_project_name(
    movement: movimentos_financeiros.Movimento, projects: list[Projeto]
) -> str | None:
    try:
