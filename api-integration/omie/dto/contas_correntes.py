...
class FinContaCorrenteListarRequest(OmiePageRequestSlugCase):
    ...
    ordenar_por: Literal[
        "CODIGO", "DATA_LANCAMENTO"
    ] = None  # string100	Ordenar o resultado da página por
    ...
    filtrar_apenas_ativo: Literal[
        "S", "N"
    ] = None  # string1	Filtrar apenas contas correntes ativas
...
class FinContaCorrenteResumoResponse(OmieResponseBodySlugCase):
    conta_corrente_lista: list[
        ContaCorrente
    ]  # conta_corrente_listaArray	Lista de contas correntes


def listar_contas_correntes(params: FinContaCorrenteListarRequest) -> Response:
    return requests.post(
        URL, json=asdict(ListarContaCorrenteRequestBody(param=[params]))
    )
